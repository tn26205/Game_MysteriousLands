...
Solver::Solver(size_t w, size_t h, vector<vector<size_t>>& x, vector<vector<size_t>>& y) : width(w), height(h), xAxisClue(x), yAxisClue(y) {
...
bool Solver::solve() {
	LineSolver lSolver;
	bool info_increased; // theo doi co the tang thong tin hay chua
	bool is_solved; // theo doi da giai quyet duoc bai toan hay chua
	...
	*/
	size_t x_mask = (1 << height) - 1;  // ktra xem mot hang da dc giai quyet hay chua.
	size_t y_mask = (1 << width) - 1;   // khi toan bo ca bit trong xStats hoac yStats
	//cua mot hang hoac cot dc set thanh 1 thi hang va cot do duoc coi la da giai quyet
	...
	while (true) {
		...
		for (size_t i = 0; i < width; ++i) {
			...
			if (!xStats[i][0]) { // neu cot chua duoc giai

				lSolver.solve(height, xStats[i][1], xStats[i][2], &xAxisClue[i]);
				//xStats[i][1]: so o day, xStats[i][2]:ssss so o trong, xAxisClue[i]:cac chi dan cua truc x
				result_solid = lSolver.getSolid(); // lay kqua so o day
				result_dot = lSolver.getDot();     // lay kqua so o trong
				...
				if (result_solid != xStats[i][1] || result_dot != xStats[i][2]) {
					...
					updateYStats(i); // cap nhat cac thong ke cua cot doc tuong ung
				}
				xStats[i][0] = ((xStats[i][1] | xStats[i][2]) == x_mask); // ktra neu tat ca ca o duoc dien == x_mask thi xStats[i][0] = true -> cot da duoc giai quyet
				if (!xStats[i][0]) {
					is_solved = false;
				}
			}

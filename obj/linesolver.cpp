...
bool LineSolver::solve (size_t block = 0, size_t position = 0, size_t parents = 0) {
	...
	while (!done) {
        ...
        // ktra xem khoi o hien tai co bi che boi o trong va co canh ke voi mot khoi solid k, neu k thi xu ly o htai
		if (!isDotCovered(bitblock) && !isSolidAbutted (bitblock)) {
			// If we're not on the last block, we move to the next one using recursion(de quy).
			...
			// If we *are* on the last block, we check if it's a possible solution, which we add.
				...
				improvedSolid &= temp; // Cap nhat ca khoi o solid da duoc xac dinh tren dong
				improvedDot |= temp; // Cap nhat cac khoi o dot da duoc xac dinh tren dong
		...
		bitblock >>= 1; // sau khi xu ly xong khoi o htai cta di chuyen sang khoi o ben phai
		++position;
		if (lastrun || isSolidRevealed(bitblock)) {
			done = true;
		}
		else if (bitblock & rightmost_possible_position) {
			lastrun = true; // xac dinh rang sap toi se la lan chay cuoi cung
		}
	}
	return true;
}
...
void LineSolver::solve(size_t l, size_t ps, size_t pd, vector<size_t> *c) {
	length = l;
	partialSolid = ps; // cac bit dai dien cho vi tri khoi solid(1) tren dong
	partialDot = pd;   // cac bit dai dien cho khoi dot(0) tren dong
	clue = c; // gan con tro c den vector<size_t> cho clue de truy cap cac kich thuoc cac khoi tren dong
	...
	// Normally we do this.
		...
		improvedSolid = (size_t)-1; /* gan bang mot so nguyen duong lon nhat co the dai dien cho tat ca
		cac khoi solid la '1', cho phep tat ca cac khoi solid co the xuat hien o bat ky vi tri nao tren dong*/
		improvedDot = 0;
		improvedDotMask = (1 << length) - 1;
		solve();
	}
}/*Đây là một hàm dùng để chuẩn bị môi trường và điều kiện cần thiết để bắt đầu giải quyết dòng bằng cách
gọi hàm solve() từ các tham số được truyền vào. Nó sẽ thiết lập các biến thành viên và tiến hành giải quyết
dòng dựa trên thông tin đã cung cấp.*/
